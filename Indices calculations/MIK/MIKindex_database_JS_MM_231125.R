...
#there is one more station than in the downloaded file
...
#3 less stations than in the download
...
str(MIK_Station)
#Convert all variables as in the downloaded file

MIK_Station<-MIK_Station%>% mutate(
  meshsize = as.integer(meshSize),
  codendmesh = as.integer(codendMesh),
  hour = as.integer(hour),
  minute = as.integer(minute),
  hauldurationminutes = as.numeric(haulDurationMinutes),
  hauldurationseconds = as.numeric(haulDurationSeconds),
  netopening = as.integer(netopening),
  netopeningarea = as.numeric(netopeningArea),
  flowefficiency = as.integer(flowEfficiency),
  netclogging = as.integer(netClogging),
  flowextrevs = as.integer(flowExtRevs),
  depthupper = as.integer(depthUpper),
  depthlower = as.numeric(depthLower),
  depthbottom = as.numeric(depthBottom),
  surtemp = as.numeric(surTemp),
  bottemp = as.numeric(botTemp),
  sursal = as.numeric(surSal),
  botsal = as.numeric(botSal)
)
...
for(i in 1992:max_YR) {
  ...
  indexSubset<-
    subset(indexALL,
           ...
           |(indexALL$startLongitude>=6&indexALL$startLongitude<9&indexALL$startLatitude>=54&
               indexALL$startLatitude<57&(indexALL$length>18|is.na(indexALL$length))))
  ...
  str(MIKindex_sumLV)
  MIKindex_sumLV$elvolFlag[is.na(MIKindex_sumLV$elvolFlag)] <- ""
  ...
  MIK_RAbun$L.sqm[is.na(MIK_RAbun$L.sqm)] <- 0
