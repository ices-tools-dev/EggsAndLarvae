...
varPOS<-c("HaulID","ShootLat","ShootLong")
...
indexSubset<-subset(indexALL,(indexALL$ShootLat>=54&indexALL$ShootLong<6)|(indexALL$ShootLat>=57&indexALL$ShootLong<9)|indexALL$ShootLong>=9|(indexALL$ShootLat<54&(indexALL$length>18|is.na(indexALL$length)))|(indexALL$ShootLong>=6&indexALL$ShootLong<9&indexALL$ShootLat>=54&indexALL$ShootLat<57&(indexALL$length>18|is.na(indexALL$length))))

# all number=NA set to 0 
indexSubset$number[is.na(indexSubset$number)]<-0
indexSubset$SubSamplingFactor[is.na(indexSubset$SubSamplingFactor)]<-1
indexSubset$number<-as.numeric(indexSubset$number)
indexSubset$SubSamplingFactor<-as.numeric(indexSubset$SubSamplingFactor)

# calculation of raised number per length class (raised by subsampling factor)

indexSubset$NumberLarvae<-indexSubset$number*indexSubset$SubSamplingFactor
